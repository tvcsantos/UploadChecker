...
import pt.com.santos.util.Cache;
import pt.com.santos.util.Pair;
import pt.com.santos.util.Persistent;
import pt.com.santos.util.StringUtilities;
import pt.com.santos.util.media.imdb.IMDB;
import pt.com.santos.util.media.tmdb.TMDbMovieSearchResult;
import pt.com.santos.util.media.tmdb.TMDbSearch;
...
public class UploadCheckerView extends FrameView {
    ...
    public UploadCheckerView(SingleFrameApplication app) {
        ...
        if (UploadCheckerApp.USER_OS != null &&
            ...
            SystemUtilities.useWindows7FileChooserIcons(
                    homeFolderIcon, upFolderIcon,
        ...
        try {
            /*searchCache = Persistent.loadCache(settingsDir.getAbsolutePath()
                                    ...
                                    SIZE_5MB);*/
            searchCache = new Cache<String, List<TMDbMovieSearchResult>>();
        } catch(Exception e) {
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(pt.com.santos.uploadchecker.UploadCheckerApp.class).getContext().getResourceMap(UploadCheckerView.class);
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(pt.com.santos.uploadchecker.UploadCheckerApp.class).getContext().getActionMap(UploadCheckerView.class, this);
    ...
    private void addMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addMenuItemActionPerformed
        ...
        FileNameExtensionFilter extensionFilter = 
                SystemUtilities.getExtensionFilter("Matroska (.mkv)", "mkv");
    ...
    private void searchResListValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_searchResListValueChanged
        ...
        final TMDbMovieSearchResult sr = (TMDbMovieSearchResult) dlm.get(index);
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        ...
        try {
            ...
            List<TMDbMovieSearchResult> sres = searchCache.get(id);
            if (sres == null) {
                new Thread() {
                    @Override
                    public void run() {
                        try {
                            List<TMDbMovieSearchResult> sres = TMDbSearch.search(id);
                            searchCache.put(id, sres);
                            /*Persistent.persist(settingsDir.getAbsolutePath()
                                    + UploadCheckerApp.FILE_SEPARATOR
                                    + "search.cache", searchCache);*/
                            dlm.clear();
                            for (TMDbMovieSearchResult sr : sres) {
                                dlm.addElement(sr);
                            }
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    TMDbMovieSearchResult m = (TMDbMovieSearchResult) dlm.get(index);
                    List<String> langs = languageCache.get(m.getMovieID());
                    if (langs == null) {
                        langs = getSpokenLanguages(m);
    ...
    private Cache<String, List<TMDbMovieSearchResult>> searchCache;
    ...
    private void downloadImages(List<TMDbMovieSearchResult> lsr,
            int maxThreads, int logBase) {
        if (lsr.isEmpty()) return;
    ...
    private class DownloadImagesThread extends Thread {
        ...
        private List<TMDbMovieSearchResult> lsr;
        private int start;
        private int total;
        //private WaitThread wt;

        public DownloadImagesThread(List<TMDbMovieSearchResult> lsr,
        ...
        public void run() {
            try {
                ...
                int count = start;
                for (TMDbMovieSearchResult sr : lsr) {
                    ...
                    DefaultListModel dlm =
                            (DefaultListModel) searchResList.getModel();
                    TMDbMovieSearchResult r = (TMDbMovieSearchResult) dlm.get(selected);
    ...
    public static List<String> getSpokenLanguages(TMDbMovieSearchResult sr) 
        throws IOException {
        String movieID = sr.getMovieID();
        if (!StringUtilities.isNullOrEmpty(movieID)) {
            return TMDbSearch.getSpokenLanguages(movieID);
        }
        
        return IMDB.getSpokenLanguages(sr.getImdbID());
    }
}